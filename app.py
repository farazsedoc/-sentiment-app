...
def chart_sentiment_bar(df: pd.DataFrame):
    ...
    fig.update_layout(**PLOTLY_LAYOUT, title="Review Count by Sentiment", height=320, bargap=0.35)
...
def chart_sentiment_over_time(df):
    ...
    try:
        ...
        line_colors = {"positive": "#00ff87", "negative": "#ff4d6d", "neutral": "#ffe600"}
        fill_colors = {"positive": "rgba(0,255,135,0.07)", "negative": "rgba(255,77,109,0.07)", "neutral": "rgba(255,230,0,0.07)"}
        fig = go.Figure()
        for sent in ["positive", "negative", "neutral"]:
            if sent in pivot.columns:
                fig.add_trace(go.Scatter(
                    x=pivot["week"], y=pivot[sent],
                    name=sent.title(), mode="lines+markers",
                    line=dict(color=line_colors[sent], width=2.5),
                    marker=dict(size=6, color=line_colors[sent]),
                    fill="tozeroy", fillcolor=fill_colors[sent],
                    hovertemplate=sent.title() + "<br>Count: %{y}<extra></extra>",
                ))
        fig.update_layout(**PLOTLY_LAYOUT, title="Sentiment Trend Over Time", height=320, hovermode="x unified")
        return fig
    except Exception:
        return None
...
def chart_rating_distribution(df: pd.DataFrame):
    if 'rating' not in df.columns:
        return None
    # Force ratings to integer and keep only valid 1-5 values
    df2 = df.copy()
    df2['rating'] = pd.to_numeric(df2['rating'], errors='coerce').dropna().astype(int)
    df2 = df2[df2['rating'].between(1, 5)]
    # Build a full 1-5 index so all star ratings always show
    counts = df2['rating'].value_counts().reindex([1, 2, 3, 4, 5], fill_value=0)
    colors = ['#ff4d6d', '#ff8c55', '#ffe600', '#9bde7e', '#00ff87']
    fig = go.Figure(go.Bar(
        x             = ['1 ⭐', '2 ⭐', '3 ⭐', '4 ⭐', '5 ⭐'],
        y             = counts.values,
        marker_color  = colors,
...
def page_live_analyzer(vectorizer, model):
    ...
    # Use a separate key for example injection to avoid widget key conflict
    ...
    example_reviews = {
        ...
        "🟡 Neutral Example" : "The product is okay. It does what it is supposed to do but nothing really stands out about it.",
    }
    ...
    with col_input:
        ...
        ex_list = list(example_reviews.items())
        for i, (label, sample) in enumerate(ex_list):
            with ex_cols[i]:
                if st.button(label.split()[0] + " " + label.split()[1], key=f"ex_{i}"):
                    st.session_state["example_text"] = sample
                    st.rerun()

        st.button("⚡ Analyze Sentiment", use_container_width=True, key="live_analyze_btn")
        st.markdown('</div>', unsafe_allow_html=True)

    with col_result:
        live_text = review_input
...
.mobile-nav-btn.active .mobile-nav-label { color: #00ff87; }
.mobile-nav-btn.active .mobile-nav-icon  { filter: drop-shadow(0 0 4px #00ff87); }
...
.top-header-logo span { color: #00ff87; }
.top-header-user {
    font-size: 0.75rem;
    color: #55556a;
    font-family: 'Space Grotesk', sans-serif;
...
def render_top_header(page, username):
    page_icons = {
        "Dashboard"        : "📊",
        "All Reviews"      : "📋",
        "Submit Review"    : "✍️",
    ...
    <div class="top-header">
        <div class="top-header-logo">Senti<span>Scope</span></div>
        <div class="top-header-page">{icon} {page}</div>
...
def render_bottom_nav(current_page):
    # Render 6 buttons in a row using Streamlit columns
    cols = st.columns(6)
    nav_map = {
        "📊": "Dashboard",
        "📋": "All Reviews",
    ...
    # Fixed bottom nav HTML (visual only)
    nav_html = '<div class="mobile-nav">'
    for icon, page in NAV_ITEMS_MOBILE:
        active_cls = "active" if current_page == page else ""
        short = labels.get(page, page)
        nav_html += f"""
        <div class="mobile-nav-btn {active_cls}">
    ...
    for col, (icon, page) in zip(cols, NAV_ITEMS_MOBILE):
        with col:
            if st.button(icon, key=f"botnav_{page}", use_container_width=True, help=page):
...
def main():
    ...
    # ── Top header ────────────────────────────────────────────────────────
    render_top_header(page, st.session_state.username)
    ...
    # ── Sign out button (below nav) ───────────────────────────────────────
    with st.sidebar:
        st.markdown("""
        <div class="sb-logo">Senti<span>Scope</span>
        <span class="sb-version">v2.4.1</span></div>
        """, unsafe_allow_html=True)
        if st.button("🚪 Sign Out", use_container_width=True, key="signout"):
            st.session_state.authenticated = False
            st.rerun()
